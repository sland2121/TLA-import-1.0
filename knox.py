"""
knox.py contains methods for creating MySQL database tables for
various knox rack types.
"""
...
def T4(TLA_dict):
    ...
    sql = "CREATE TABLE %s like TLAKnoxT4Template" %TLA_dict['TLA_name']
    cursor.execute(sql)
    
    #copy entire table
    sql = "INSERT INTO %s SELECT * FROM TLAKnoxT4Template"%TLA_dict['TLA_name']
    cursor.execute(sql)
